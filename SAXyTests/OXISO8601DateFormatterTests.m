/**
  OXISO8601DateFormatterTests.m
  SAXy

  Attempting to use ISO8601DateFormatter with SAXy but ran into problems...can't round-trip with own output

  TASKS COMPLETED:
  * embedded ISO8601DateFormatter interface and class in this file
  * prefixed global names with OX_ to avoid namespace conflicts
  * set UTC as default time zone

  PROBLEMS:
  * always outputs zulu time (i.e. 1937-04-21T11:10:09Z)
  * when reading it's own output, the formatter corrects for local time, ignoring the 'Z' suffix

  Created by Richard on 3/13/13.
...
////////////////////////////////////////////////////////////////////////////////////////
#pragma mark - ISO8601DateFormatter - renamed to avoid name collisions
////////////////////////////////////////////////////////////////////////////////////////
 ...
 */
enum {
	OX_ISO8601DateFormatCalendar,
	OX_ISO8601DateFormatOrdinal,
	OX_ISO8601DateFormatWeek,
};
typedef NSUInteger OX_ISO8601DateFormat;

//The default separator for time values. Currently, this is ':'.
extern unichar OX_ISO8601DefaultTimeSeparatorCharacter;

@interface OX_ISO8601DateFormatter: NSFormatter
{
...
@property OX_ISO8601DateFormat format;
...
#endif
unichar OX_ISO8601DefaultTimeSeparatorCharacter = DEFAULT_TIME_SEPARATOR;

//Unicode date formats.
#define OX_ISO_CALENDAR_DATE_FORMAT @"yyyy-MM-dd"
//#define OX_ISO_WEEK_DATE_FORMAT @"YYYY-'W'ww-ee" //Doesn't actually work because NSDateComponents counts the weekday starting at 1.
#define OX_ISO_ORDINAL_DATE_FORMAT @"yyyy-DDD"
#define OX_ISO_TIME_FORMAT @"HH:mm:ss"
#define OX_ISO_TIME_WITH_TIMEZONE_FORMAT  OX_ISO_TIME_FORMAT @"Z"
//printf formats.
#define OX_ISO_TIMEZONE_UTC_FORMAT @"Z"
#define OX_ISO_TIMEZONE_OFFSET_FORMAT @"%+.2d%.2d"

@interface OX_ISO8601DateFormatter(UnparsingPrivate)
...
@implementation OX_ISO8601DateFormatter
...
- (id) init {
	if ((self = [super init])) {
		...
		format = OX_ISO8601DateFormatCalendar;
		timeSeparator = OX_ISO8601DefaultTimeSeparatorCharacter;
		includeTime = NO;
		parsesStrictly = NO;
	}
	return self;
...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
	...
	if (strict) timeSep = OX_ISO8601DefaultTimeSeparatorCharacter;
...
- (NSDate *) dateFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
    ...
    NSLog(@"parsingCalendar.timeZone: %@", parsingCalendar.timeZone);
...
- (NSString *) stringFromDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	switch (self.format) {
		case OX_ISO8601DateFormatCalendar:
			return [self stringFromDate:date formatString:OX_ISO_CALENDAR_DATE_FORMAT timeZone:timeZone];
		case OX_ISO8601DateFormatWeek:
...
- (NSString *) stringFromDate:(NSDate *)date formatString:(NSString *)dateFormat timeZone:(NSTimeZone *)timeZone {
	if (includeTime)
		dateFormat = [dateFormat stringByAppendingFormat:@"'T'%@", [self replaceColonsInString:OX_ISO_TIME_FORMAT withTimeSeparator:self.timeSeparator]];
	...
	if (includeTime) {
		...
		if (offset == 0)
			str = [str stringByAppendingString:OX_ISO_TIMEZONE_UTC_FORMAT];
		else
			str = [str stringByAppendingFormat:OX_ISO_TIMEZONE_OFFSET_FORMAT, offset / 60, offset % 60];
	}
...
- (NSString *) weekDateStringForDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	...
	if(includeTime) {
		...
		unichar timeSep = self.timeSeparator;
		if (!timeSep) timeSep = OX_ISO8601DefaultTimeSeparatorCharacter;
		formatter.dateFormat = [self replaceColonsInString:OX_ISO_TIME_WITH_TIMEZONE_FORMAT withTimeSeparator:timeSep];
...
@interface OX_ISO8601Date_Data : NSObject
@property(strong)NSDate *date1;
@property(strong)NSDate *date2;
@property(strong)NSDate *date3;
@end

@implementation OX_ISO8601Date_Data  @end

////////////////////////////////////////////////////////////////////////////////////////
#pragma mark - tests
////////////////////////////////////////////////////////////////////////////////////////

@interface OXISO8601DateFormatterTests : SenTestCase  @end

@implementation OXISO8601DateFormatterTests

- (void)testISODate_unusable
{
    OX_ISO8601DateFormatter *dateFormatter = [[OX_ISO8601DateFormatter alloc] init];
    dateFormatter.defaultTimeZone = [NSTimeZone timeZoneWithAbbreviation:@"UTC"];//localTimeZone];
    //dateFormatter.parsesStrictly = YES;
    dateFormatter.includeTime = YES;
    
    //test formatter stand-alone:
    NSDate *date1 = [dateFormatter dateFromString:@"1937-04-21T11:10:09'Z'"];   //HACK TO MATCH OUTPUT, SHOULD NOT REQUIRE SINGLE QUOTES
    //NSDate *date1 = [dateFormatter dateFromString:@"1937-04-21T11:10:09Z"];   //THIS SHOULD WORK BUT DOESN'T
    NSString *str1 = [dateFormatter stringFromDate:date1];
    
    STAssertEqualObjects(@"1937-04-21T11:10:09Z", str1, @"BROKEN zulu round trip");
    

    //test formatter with SAXy:
    OXmlMapper *mapper = [[OXmlMapper mapper] elements:@[  [OXmlElementMapper rootXPath:@"/iso" type:[OX_ISO8601Date_Data class]]  ]];
    
    OXmlReader *reader = [OXmlReader readerWithMapper:mapper];
    reader.context.logReaderStack = NO;                                               //'YES' -> log mapping process
    [reader.context.transform registerDefaultDateFormatter:dateFormatter];             //make ISO8601DateFormatter the default date formatter
    
    NSString *xml1 = @"<iso><date1>1937-04-21T11:10:09'Z'</date1><date2>1937-04-21T11:10:09+UTC</date2><date3>1937-04-21T11:10:09</date3></iso>";
    
    OX_ISO8601Date_Data *iso = [reader readXmlText:xml1];                               //reads xml
    ...
    NSString *xml2 = [writer writeXml:iso prettyPrint:NO];                              //writes xml
    
    //THE OUTPUT NEVER EQUALS THE INPUT - I.E. THIS FORMATTER CAN'T READ IT'S OWN OUTPUT AND THUS CAN'T ROUND-TRIP!
    NSString *xml1Zulu_HACK = @"<iso><date1>1937-04-21T11:10:09Z</date1><date2>1937-04-21T11:10:09Z</date2><date3>1937-04-21T11:10:09Z</date3></iso>";
    STAssertEqualObjects(xml1Zulu_HACK, xml2, @"input normalized to Zulu time equals output xml");
